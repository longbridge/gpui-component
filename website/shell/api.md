...
Throughout the tables below, four forms recur. Each is an object whose only members are factories, mirroring the associated function on the Rust side:

| Form | Factory | Mirrors |
| --- | --- | --- |
| Type table | `.new(id)`, sometimes with a second argument | `Button::new(id)` |
| Part table | `.new()` — a sub-part with no identity of its own | `ProgressTrack::new()` |
| Indexed table | `.new(id, index)`, one-based | `TableRow::new(id, index)` |
| State table | `.new(state)` or `.new(options)` | `Input::new(state)` |
...
| `svg(path)` | Function | A vector image from the application root, tinted by the surrounding text color |
| `image(path)` | Function | A full-color image from the application root, colors preserved |
| `PathBuilder` | Const | `fill()` and `stroke(width)`, each opening a path under construction |
| `Background` | Const | `solid`, `stop`, `linear_gradient`, `pattern_slash`, `checkerboard` |
...
| Name | Form | What it is |
| --- | --- | --- |
| `View` | Abstract class | The base class of every view; subclass it and default-export the subclass |
| `ViewClass` | Type | A concrete `View` subclass, as `cx.new` takes it |
| `Entity` | Interface | Retained ownership of one nested view: `set_props(props)`, `release()` |
| `Props` | Type | The property bag handed to `init` and to `cx.new` |
...
| Name | Form | What it is |
| --- | --- | --- |
| `store` | Const | Key-value storage that survives a restart, persisted on every write |
| `Store` | Interface | `get(key)`, `set(key, value)`, `remove(key)`, `keys()`, `flush()` |
| `Json` | Type | Everything the store can persist, and nothing else |
...
| Name | Form | What it is |
| --- | --- | --- |
| `native(module)` | Function | A module the host registered in Rust; throws and names the ones that exist when it finds none |
| `NativeModules` | Interface | Empty here — an application declares its own modules into it and `native("…")` becomes typed |

### Scheduling

| Name | Form | What it is |
| --- | --- | --- |
| `Task` | Interface | A running task: `cancel()`, `is_done()` |
| `TaskOptions` | Interface | `owner` — the view the task is cancelled with, or `null` to outlive every view |
| `Timer` | Interface | `after(ms, handler, opts?)` and `every(ms, handler, opts?)` |

### Focus and component shapes

| Name | Form | What it is |
| --- | --- | --- |
| `FocusHandleHandle` | Interface | A focus target the script owns: `focus()`, `is_focused()`, `release()` |
| `ComponentType` | Interface | `new(id)` — a component identified across renders |
| `PartType` | Interface | `new()` — a sub-part with no identity of its own |
| `IndexedComponentType` | Interface | `new(id, index)` — a component whose one-based position is announced |

### Shared types

| Name | Form | What it is |
| --- | --- | --- |
| `Length` | Type | A number (pixels), `"12px"`, `"1.5rem"`, `"50%"` or `"auto"` |
| `DefiniteLength` | Type | The same without `"auto"` |
| `AbsoluteLength` | Type | Pixels or rems only |
| `LengthString` | Type | The string forms of a length |
| `Color` | Type | A `ColorToken` name, or a `#rgb` / `#rrggbb` / `#rrggbbaa` literal |
| `ColorToken` | Type | The seventeen semantic tokens the installed palette defines |
| `Role` | Type | An accessibility role, mirroring `gpui::Role` in snake_case |
| `Anchor` | Type | Which corner of an anchored surface is pinned to its trigger |
| `MouseButton` | Type | `"left"`, `"right"` or `"middle"` |
| `Phase` | Type | `"render"`, `"event"`, `"task"`, `"layout"` or `"none"` |
| `SheetSide` | Type | Which edge the sheet is anchored to |
| `DialogOptions` | Interface | `escape_dismissable`, `backdrop_dismissable` |
| `ToastOptions` | Interface | `title`, `description`, `level`, `timeout`, `id` |
| `ClickEvent` | Interface | `click_count`, `modifiers` |
| `MouseMoveEvent` | Interface | `position`, `local_position`, `bounds`, `modifiers` |
| `Modifiers` | Interface | `shift`, `control`, `alt`, `platform` |
| `Point` | Interface | `x`, `y` |
| `ElementBounds` | Interface | A `Point` with `width` and `height` |
| `MotionProperty` | Type | `"opacity"`, `"width"`, `"height"`, `"left"`, `"top"` |
| `MotionEasing` | Type | `"linear"`, `"ease-in"`, `"ease-out"`, `"ease-in-out"` |
| `TransitionPolicy` | Interface | `duration`, `delay`, `easing` |
| `SpringPolicy` | Interface | `response`, `damping`, `epsilon` |
| `Path` | Interface | Immutable native geometry produced by `PathBuilder.build()` |
| `PathCoordinate` | Type | Pixels, or a percentage of the painted element's bounds |
| `BackgroundValue` | Interface | A reusable native background: `opacity(factor)`, `color_space(space)` |
| `BackgroundStop` | Interface | One gradient stop, from `Background.stop(color, percentage)` |
...
| Name | Form | What it is |
| --- | --- | --- |
| `h_flex()` | Function | A row |
| `v_flex()` | Function | A column |
| `h_resizable(id)` | Function | A row of panes with draggable dividers; sizes live in the window under the id |
| `v_resizable(id)` | Function | The same, stacked |
| `resizable_panel()` | Function | One pane of a resizable group, and legal nowhere else |

### Controls

| Name | Form | What it is |
| --- | --- | --- |
| `Button` | Type table | Activation, focus, disabled and selected state |
| `Link` | Type table | An external HTTP(S) resource opened through the system browser |
| `Checkbox` | Type table | A controlled toggle; draw the indicator yourself |
| `Switch` | Type table | A controlled switch |
| `Radio` | Type table | One option in a group; reports `true` only, never a deselection |
| `Toggle` | Type table | A button that stays down |
| `RadioGroup` | Type table | A set of radios announced as one group; holds no selection |
| `ToggleGroup` | Type table | A set of toggles announced as a toolbar |
| `Tabs` | Type table | A tab list that holds no selection of its own |
| `Tab` | Type table | One tab: `selected(...)` in, `on_click(...)` out |
| `Progress` | Type table | The announcement, not the bar; `Progress.new(...)` alone draws nothing |
| `ProgressTrack` | Part table | The groove: a plain element you size and color |
| `ProgressIndicator` | Part table | The filled part; set its width from the percentage you announced |
| `SliderState` | State table | Retained slider state, and where a drag writes |
| `Slider` | State table | The root: announces the value and owns the release |
| `SliderTrack` | State table | The press and drag surface |
| `SliderIndicator` | State table | The groove, and the box every pointer position is measured against |
| `SliderThumb` | State table | The knob; the shell gives it a place, you give it a look |
...
| Name | Form | What it is |
| --- | --- | --- |
| `InputState` | State table | Retained text state: `InputState.new({ placeholder, value })` |
| `Input` | State table | The frame around retained text state |
| `NumberInput` | State table | A spinbutton over the same `InputState`, with three slots that all carry weight |
| `TextareaState` | State table | Retained multi-line text state; `rows` is an option |
| `Textarea` | State table | The frame around retained multi-line state |
| `OtpState` | State table | Retained one-time-code state; the length is fixed when it is created |
| `OtpInput` | State table | A fixed-length code whose cells the shell draws and the script styles |
...
| Name | Form | What it is |
| --- | --- | --- |
| `Collapsible` | Part table | Renders its `content` slot only while `open`; no role, chevron or trigger |
| `Popover` | Type table | A surface anchored to a trigger and opened by a press |
| `HoverCard` | Type table | The same, opened by resting the pointer, with its own open state |
| `Popup` | Type table | The bare anchored surface: `Popup.new(id, trigger)`, opened by filling `content` |
| `Select` | Type table | A combobox root: the role, the announced open state, the keyboard — none of the picture |
| `Combobox` | Type table | The same root, announced as a combobox whose trigger is an editable field |
| `DatePicker` | Type table | A date-picker root: `DatePicker.new(id, focus_handle)`; it holds no date |
...
| Name | Form | What it is |
| --- | --- | --- |
| `Table` | Type table | A semantic table root, composed the way HTML composes one |
| `TableHeader` | Type table | The header row group |
| `TableBody` | Type table | The body row group |
| `TableRow` | Indexed table | One row: `TableRow.new(id, row_index)`, one-based |
| `TableHead` | Indexed table | One column header, one-based |
| `TableCell` | Indexed table | One data cell, one-based |
| `TableCaption` | Type table | The visual slot a caption belongs in; it carries no caption role |
| `v_virtual_list(…)` | Function | A vertical list that describes only what is on screen |
| `h_virtual_list(…)` | Function | The same along the other axis; `item_sizes` are widths |
| `VirtualListScrollHandle` | State table | A virtual list's scroll position, kept across frames |
| `Scrollbar` | Type table | `new(id)`, `horizontal(id)`, `vertical(id)` — a bar you place yourself |
...
| Name | Form | What it is |
| --- | --- | --- |
| `set_theme(theme)` | Function | Replaces `gpui-base`'s active semantic tokens with an application-owned theme |
| `Theme` | Interface | What `cx.theme()` answers: the semantic tokens plus `appearance` and `is_dark` |
| `SemanticThemeTokens` | Interface | `colors`, `spacing`, `radius` |
| `ColorTokens` | Type | One `Color` per semantic role |
| `SpacingTokens` | Interface | `xxs` `xs` `sm` `md` `lg` `xl` `xxl` |
| `RadiusTokens` | Interface | `none` `sm` `md` `lg` `xl` `full` |
...
| Name | Form | What it is |
| --- | --- | --- |
| `GroupAxis` | Type | `"horizontal"` or `"vertical"`, announced rather than drawn |
| `ScrollbarMode` | Type | `"scrolling"`, `"hover"` or `"always"` |
| `ItemRange` | Interface | A virtual list's visible items, as a half-open `[start, end)` |
| `SliderValue` | Type | A number, or `[start, end]` for a range slider |
| `PopupType`, `DatePickerType`, `ScrollbarType` | Interface | The factory shapes of the three types whose constructor takes more than an id |

## `"gpui-fps"`

| Name | Form | What it is |
| --- | --- | --- |
| `fps_monitor()` | Function | The native `gpui-fps` HUD, shared once per window and pinned to the top right |
