...
**A dialog is a view, not an element.** `confirm.js` default-exports a `View` class with its own `init` and `render`; `main.js` opens it by handing the class itself to `cx.open_dialog(ConfirmClear, { props: { count } })`. See [Overlays](./overlays.md).
