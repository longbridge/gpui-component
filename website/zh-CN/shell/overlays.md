...
打开或关闭浮层会修改窗口，而渲染阶段正在读它。GPUI 的借用模型无法表达"脚本在这里可以 notify、在那里不行"，所以运行时显式携带 [phase](./state.md#phase)，每一个浮层入口都拒绝 `render`、`layout`，以及根本不在任何宿主调用中的情形——最后这种情况下也没有窗口可以触达。
...
见 [快速开始](./getting-started.md#在-rust-宿主中嵌入运行时)。
