...
**标签页带的是命令，不是处理器。** chrome handler 每个容器每帧调用一次，只要 dock 还在屏幕上就一直如此，所以在里面注册的回调会不断堆积。`select_tab(group, tab.index)` 与 `close_panel(group, tab.id)` 完全不携带脚本值——它们只是指名某个容器、以及要请它做什么。
