...
| --- | --- | --- | --- | --- |
| 443 节点 | 1.5 ms | 1.3 ms | **0** | 2.8 ms |
| 2,103 节点 | 5.9 ms | 7.3 ms | **0** | 13.2 ms |
| 4,203 节点 | 10.2 ms | 11.8 ms | **0** | 22.0 ms |
| 8,403 节点 | 20.8 ms | 26.4 ms | **0** | 47.2 ms |

```bash
cargo test -p gpui-shell --release --test benchmark -- --ignored --nocapture
```

最后一列是前两列相加：如果每一帧都要重新描述一遍界面，那一帧就要花这么多。它在每一档都是真实值的 **1.8 到 2.1 倍**，而这个倍数就是快照买到的全部东西——无论规模多大，"描述界面"都占了朴素做法中大约一半的帧成本。

那个 0 是断言，不是观测结果。它在表中每一档都成立；一旦某次重绘进入了 VM，[基准测试会直接失败](./engine.md#那次实测)，而不是仅仅变慢一点。

这张表同时也说清了天花板在哪，而它不在 JavaScript 一侧：8,403 节点时，一帧要 26 ms，其间没有任何脚本在跑。超过几千个节点之后，账单来自 Rust 侧的物化、布局与绘制，该上的手段是虚拟化，而不是换一个更快的引擎。绝对数值取自 Apple Silicon 上的 release 构建，会随机器变化；倍数与那个 0 不会。
