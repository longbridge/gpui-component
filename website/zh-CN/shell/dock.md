...
只能铺满整个窗口的视图算不上一个应用。**dock area** 把脚本视图变成*面板*：可拖动、可停靠、可放大，重启之后仍然停在用户上次放的位置。
...
`gpui_base::dock` 已经把停靠系统里难做的那一半做好了：一棵**纯数据**的布局树、一个能按持久化文件里的名字重建面板的 `PanelRegistry`，以及跟着每块面板走的一份 payload。容器用稳定的 node id 寻址，面板用稳定的 panel id 寻址，因此一次拖动改的是一个值，而不是拆掉再重建一堆视图。
...
面板就是恰好被 dock 拿在手里的一个视图。`add_panel` 接收这个视图并说明它去哪里：
...
面板自己的状态会跟着位置一起走。视图类上有两个可选方法负责这件事：

| 方法 | 何时调用 | 说明 |
| --- | --- | --- |
| `serialize()` | 保存布局时 | 运行时**没有 Host 调用**：返回纯数据，别碰别的——不要碰 entity，不要碰 `cx` |
| `deserialize(data)` | 视图刚重建之后 | 有一次真正的 Host 调用，因此可以碰 entity |
...
每一个都在 GPUI 的 layout pass 内部被调用，每个容器每帧一次，拿到的是 base **已经解析好的**状态——从不包含拖拽事件、鼠标位置或命中测试，因为 base 会把这些自己挂到拿回去的元素上。
...
chrome handler **每个容器每帧调用一次**，只要 dock 还在屏幕上就一直如此。在里面注册的脚本回调，会归属于一个能站上几千帧的 snapshot，而标签页每一帧都会重建——于是二十个标签页、一分钟的使用，就会留下几万个再也够不着、也不会被释放的 JavaScript 函数。所以在写下它的那一行就会被拒绝。
