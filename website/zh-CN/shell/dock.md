...
::: warning 还没有对外公开
`gpui_shell::dock` 目前是 crate-private。没有东西在驱动它：脚本还不能贡献面板，也还没有 Host 会加载插件；此时把 `ScriptPanel` 或 `register_panel` 公开出去，等于对一个从没有调用者用过的 API 作出承诺。这一页讲的是里面有什么、它在等什么——今天还调用不到。
:::

## base 已经提供的部分

`gpui_base::dock` 早就有了插件系统里难做的那一半：一棵**纯数据**的布局树、一个能按名字从持久化文件里重建面板的 `PanelRegistry`，以及跟着每块面板走的一份 `serde_json::Value`。它缺的只是"让面板来自 Host 二进制之外"的办法，而这个模块补的正是这个缺口。

## 它长什么样

`ScriptPanel` 只实现 `gpui_base::dock::Panel`——也就是行为——再往上的那一层不碰。它的标题、工具栏与菜单由一层 skin 用脚本自己的元素画出来；一旦牵扯到面板，"呈现权在脚本"指的就是这件事。它的名字会被 intern 且总是带前缀，所以脚本面板不可能和恰好同名的 Host 面板撞车。
...
| `serialize` | 布局正在保存 | 拿到的是 `&App` 而不是 `&mut Window`——这是一次读取，没有 CallScope，所以脚本的 `serialize()` 必须是返回纯值、不回调 Host 的方法 |
| `deserialize` | 紧跟在 `build` 之后 | 这是一次真正的 Host 调用，可以开 scope、可以碰实体 |
...
- **一个调用者。** 等有东西真正驱动它——一个会挂载面板的插件模型，或者一个提出这个需求的 Host——这个模块才会公开。在那之前就公开，等于在没有任何使用经验的情况下把 API 的形状定死。
