...
export default class Hello extends View {
  ...
  render() {
    return v_flex()
      ...
      .gap(12)
      .bg("background")
      .child(text(`Clicked ${this.clicks} times`).text_color("foreground"))
      .child(
        Button.new("click")
          .h(28)
          .px(12)
...
`--watch` 每秒轮询应用目录四次，对一串连续写入去抖 200 ms，然后重载。一次重载会重新读取**每一个**模块，入口也在内——一个悄悄用了旧 import 的热重载比没有热重载更糟，因为它看起来是成功的。
