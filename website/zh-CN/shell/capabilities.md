...
值缓存在内存里，因为 `get` 在 `render` 里也可达，每帧读一次文件是荒唐的。**每次写入立即持久化**，先写临时文件再改名覆盖目标——所以写到一半崩溃留下的是之前完整的配置，而不是一个被截断的文件。因此 `flush` 不必调用；它留在 API 里，是为了将来写入变成可 await 的 promise 时充当持久化屏障。
