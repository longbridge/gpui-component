...
再往前的方向是 **template cache**：把一份描述拆成可复用的结构与其中的动态槽位，让只有取值变化时去填槽位，而不是重跑 builder。它尚未实现，和这个运行时[还没有花出去](./elements.md#还没有的东西)的其它杠杆并列。

已经实现的是判断它值不值得做的那个读数。`structure_repeats()` 与 `structure_changes()` 统计一次重建产出的结构与它替换掉的那份是否相同——只有其中的取值不同——也就是这类缓存能达到的上限。在一块由实时行情驱动的二十行看板上，每一次重建的结构都相同。所以如果你自己的面板报出来的比例很低，这件事本身就值得知道：你以为只有一个数字在变，实际上有东西在改变结构。
