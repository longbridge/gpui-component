...
<img class="architecture-light" src="/shell-change-cost-light.svg" alt="三条泳道，长条按同一比例绘制。视图读到的东西没变：没有长条，不执行任何脚本，这一帧重放已经发布的描述。取值变了，也就是今天的情形：无论变化多小，整块面板都被重新描述一遍，0.315 毫秒。同样的变化改为填充一份早先记录的结构：0.060 毫秒，约为五分之一——这是实测值，而且脚本刻意触达不到。">
<img class="architecture-dark" src="/shell-change-cost-dark.svg" alt="三条泳道，长条按同一比例绘制。视图读到的东西没变：没有长条，不执行任何脚本，这一帧重放已经发布的描述。取值变了，也就是今天的情形：无论变化多小，整块面板都被重新描述一遍，0.315 毫秒。同样的变化改为填充一份早先记录的结构：0.060 毫秒，约为五分之一——这是实测值，而且脚本刻意触达不到。">
