...
上面这组——`on_key_down`、`on_key_up`、四个指针事件、`on_action` 与 `key_context`——只在 `div`、`h_flex`、`v_flex` 上接线，也就是 shell 完全拥有的那几个元素。其余组件各自构建自己的 base 类型并挂自己的监听器，所以写在 `Button` 上的处理器会被记录下来但永远到不了 GPUI；日志里会说明这一点。把它包一层，写在外层元素上。
