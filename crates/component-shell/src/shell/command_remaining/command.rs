...
pub(super) fn register(registry: &mut ComponentRegistry) -> Result<(), RegistryError> {
    ...
    registry.register(ComponentDescriptor { name:"CommandItem", constructors:vec![ConstructorDescriptor::new("CommandItem", vec![ArgumentDescriptor::new("label", ArgumentSchema::String)], |args| match args { [ComponentArgument::String(label)] if !label.trim().is_empty() => Ok(ComponentPayload::new(ItemPayload(label.clone()))), _ => Err("CommandItem expects non-empty label".into()) })], methods:vec![
        ...
        MethodDescriptor::new("keyword", vec![ArgumentDescriptor::new("keyword", ArgumentSchema::String)], |args| match args { [ComponentArgument::String(value)] if !value.trim().is_empty() => Ok(ComponentPayload::new(ItemOp::Keyword(value.clone()))), _ => Err("CommandItem.keyword expects non-empty text".into()) }).documented("Sets the item search keyword."),
        MethodDescriptor::new("checked", vec![ArgumentDescriptor::new("checked", ArgumentSchema::Boolean)], |args| match args { [ComponentArgument::Boolean(value)] => Ok(ComponentPayload::new(ItemOp::Checked(*value))), _ => Err("CommandItem.checked expects boolean".into()) }).documented("Sets the item checked state."),
        MethodDescriptor::new("action", vec![ArgumentDescriptor::new("action", ArgumentSchema::String)], |args| match args { [ComponentArgument::String(value)] if !value.trim().is_empty() => Ok(ComponentPayload::new(ItemOp::Action(value.clone()))), _ => Err("CommandItem.action expects non-empty action id".into()) }).documented("Dispatches the named shell action when selected."),
    ...
    registry.register(ComponentDescriptor { name:"Command", constructors:vec![ConstructorDescriptor::new("Command", vec![ArgumentDescriptor::new("state", ArgumentSchema::Entity("CommandState"))], |args| match args { [argument @ ComponentArgument::Entity { .. }] => Ok(ComponentPayload::new(CommandPayload(argument.clone()))), _ => Err("Command expects CommandState".into()) })], methods:vec![
        ...
        MethodDescriptor::new("placeholder", vec![ArgumentDescriptor::new("placeholder", ArgumentSchema::String)], |args| match args { [ComponentArgument::String(value)] => Ok(ComponentPayload::new(CommandOp::Placeholder(value.clone()))), _ => Err("Command.placeholder expects text".into()) }).documented("Sets the command search placeholder."),
        MethodDescriptor::new("maxHeight", vec![ArgumentDescriptor::new("pixels", ArgumentSchema::Number)], |args| match args { [ComponentArgument::Number(value)] if value.is_finite() && *value > 0. && *value <= f32::MAX as f64 => Ok(ComponentPayload::new(CommandOp::MaxHeight(*value as f32))), _ => Err("Command.maxHeight expects positive finite pixels".into()) }).documented("Sets the command results maximum height."),
