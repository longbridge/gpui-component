...
use gpui_shell::{
    ...
    gpui_component::{Disableable as _, button::Button, native_menu::NativeMenu},
};
use std::sync::Arc;
...
fn require(parent: &str, actual: Option<&'static str>, allowed: &[&str]) -> anyhow::Result<()> {
    anyhow::ensure!(
...
fn reject_style(style: gpui::StyleRefinement, name: &str) -> anyhow::Result<()> {
    anyhow::ensure!(
        ...
        "{name} is native-menu data and does not support shell style"
    );
    Ok(())
}
fn resolve_item<'a>(
...
impl ComponentMaterializer for ItemMat {
    fn materialize(&self, mut r: MaterializeRequest<'_>) -> anyhow::Result<gpui::AnyElement> {
        ...
        reject_style(r.take_style(), "NativeMenuItem")?;
        for child in r.take_typed_children() {
            require("NativeMenuItem", child.component_name(), &[])?;
        }
...
impl ComponentMaterializer for SeparatorMat {
    fn materialize(&self, mut r: MaterializeRequest<'_>) -> anyhow::Result<gpui::AnyElement> {
        reject_style(r.take_style(), "NativeMenuSeparator")?;
        for child in r.take_typed_children() {
            require("NativeMenuSeparator", child.component_name(), &[])?;
        }
        Ok(Carrier::new(Entry::Separator).into_any_element())
    }
}
struct TriggerMat;
impl ComponentMaterializer for TriggerMat {
    fn materialize(&self, mut r: MaterializeRequest<'_>) -> anyhow::Result<gpui::AnyElement> {
        let trigger = r
            .payload()
        ...
        let effects = r.resolve_callback(&reporter.0)?.window_effects();
        ...
        button.style().refine(&r.take_style());
...
pub(super) fn register(registry: &mut ComponentRegistry) -> Result<(), RegistryError> {
    ...
    registry.register(ComponentDescriptor {
        ...
        methods: vec![],
        typescript: TypeScriptDescriptor::new("Typed native-menu separator data."),
...
mod tests {
    ...
    fn typed_lane_is_closed() {
        ...
        assert!(require("NativeMenuTrigger", None, &["NativeMenuItem"]).is_err());
    }
    #[test]
