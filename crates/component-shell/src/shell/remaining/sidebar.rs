...
use gpui_shell::{
    ...
    gpui_component::{
        Side,
        sidebar::{
            Sidebar, SidebarCollapsible, SidebarFooter, SidebarHeader, SidebarMenu,
            SidebarMenuItem, SidebarToggleButton,
        },
    },
};

use super::typed::{OpaqueChildElement, take_opaque};
...
fn side_method<T: 'static + Send + Sync>(
    ...
    .documented("Sets the physical side occupied by the sidebar control.")
}
...
fn on_click_method(owner: &'static str) -> MethodDescriptor {
    ...
    .documented("Invokes the callback when the control is activated.")
}
...
fn apply_edge_selected<E: gpui_shell::gpui_component::Selectable>(edge: E, selected: bool) -> E {
...
impl ComponentMaterializer for MenuItemMaterializer {
    fn materialize(&self, mut request: MaterializeRequest<'_>) -> anyhow::Result<gpui::AnyElement> {
        ...
        for mut child in request.take_typed_children() {
            ...
            nested.push(take_opaque::<SidebarMenuItem>(
                &mut element,
                "SidebarMenuItem",
            )?);
        }
...
impl ComponentMaterializer for MenuMaterializer {
    fn materialize(&self, mut request: MaterializeRequest<'_>) -> anyhow::Result<gpui::AnyElement> {
        ...
        for mut child in request.take_typed_children() {
            ...
            let mut element = request.materialize_child(&mut child)?;
            menu = menu.child(take_opaque::<SidebarMenuItem>(
                &mut element,
                "SidebarMenuItem",
            )?);
        }
        Ok(OpaqueChildElement::new(menu).into_any_element())
    }
}
...
impl ComponentMaterializer for SidebarMaterializer {
    fn materialize(&self, mut request: MaterializeRequest<'_>) -> anyhow::Result<gpui::AnyElement> {
        ...
        for mut child in request.take_typed_children() {
            ...
            let mut element = request.materialize_child(&mut child)?;
            sidebar = sidebar.child(take_opaque::<SidebarMenu>(&mut element, "SidebarMenu")?);
        }
...
pub(super) fn register(registry: &mut ComponentRegistry) -> Result<(), RegistryError> {
    registry.register(ComponentDescriptor {
        ...
        constructors: vec![id_constructor("SidebarMenuItem", "label")],
        methods: vec![
        ...
        typescript: TypeScriptDescriptor::new("A typed navigation row accepted by SidebarMenu. Shell style operations are unsupported because the native SidebarMenuItem is not Styled."),
        materializer: Arc::new(MenuItemMaterializer),
    })?;
    registry.register(ComponentDescriptor {
        name: "SidebarMenu",
        constructors: vec![nullary("SidebarMenu")],
        methods: vec![],
        typescript: TypeScriptDescriptor::new(
            "A typed Sidebar menu accepting SidebarMenuItem children.",
        ),
        materializer: Arc::new(MenuMaterializer),
    })?;
    let selected = || {
        vec![bool_method(
            "Sidebar edge",
    ...
    registry.register(ComponentDescriptor {
        name: "SidebarHeader",
        constructors: vec![nullary("SidebarHeader")],
        methods: selected(),
        typescript: TypeScriptDescriptor::new(
            "A styled sidebar header accepting ordinary children.",
        ),
        materializer: Arc::new(HeaderMaterializer),
    })?;
    registry.register(ComponentDescriptor {
        name: "SidebarFooter",
        constructors: vec![nullary("SidebarFooter")],
        methods: selected(),
        typescript: TypeScriptDescriptor::new(
            "A styled sidebar footer accepting ordinary children.",
        ),
        materializer: Arc::new(FooterMaterializer),
    })?;
    registry.register(ComponentDescriptor {
        name: "Sidebar",
        constructors: vec![id_constructor("Sidebar", "id")],
        methods: vec![
            ...
            ).documented("Sets the sidebar collapse behavior."),
            bool_method("Sidebar", "collapsed", "Sets the controlled collapsed state.", SidebarOp::Collapsed),
        ],
        typescript: TypeScriptDescriptor::new("A typed application sidebar accepting SidebarMenu children and named header/footer slots."),
        materializer: Arc::new(SidebarMaterializer),
    })?;
    registry.register(ComponentDescriptor {
        name: "SidebarToggleButton",
        constructors: vec![nullary("SidebarToggleButton")],
        methods: vec![
        ...
        materializer: Arc::new(ToggleMaterializer),
    })?;
    Ok(())
}
...
mod tests {
    ...
    fn typed_carrier_rejects_double_consumption() {
        let mut element = OpaqueChildElement::new(SidebarMenu::new()).into_any_element();
        assert!(take_opaque::<SidebarMenu>(&mut element, "SidebarMenu").is_ok());
        assert!(take_opaque::<SidebarMenu>(&mut element, "SidebarMenu").is_err());
    }
    ...
    fn common_selected_state_reaches_native_header_and_footer_selection() {
        use gpui_shell::gpui_component::Selectable as _;
