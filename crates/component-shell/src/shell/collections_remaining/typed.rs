...
pub(super) fn take<T: 'static>(e: &mut gpui::AnyElement, n: &str) -> anyhow::Result<T> {
    e.downcast_mut::<Carrier<T>>()
        .ok_or_else(|| anyhow::anyhow!("{n} materialized an incompatible child"))?
        .0
        .take()
        .ok_or_else(|| anyhow::anyhow!("{n} child already consumed"))
}
