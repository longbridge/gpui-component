...
impl ComponentMaterializer for Materializer {
    fn materialize(&self, mut request: MaterializeRequest<'_>) -> anyhow::Result<gpui::AnyElement> {
        ...
        let on_ok = callback(|op| if let Op::OnOk(v) = op { Some(v) } else { None })?;
        let on_cancel = callback(|op| {
            if let Op::OnCancel(v) = op {
                Some(v)
            } else {
                None
            }
                            ...
                            match kind {
                                ...
                                Kind::Notification => {
                                    ...
                                    let kind = operations
                                        .iter()
                                        .filter_map(|op| {
                                            if let Op::Kind(v) = op { Some(*v) } else { None }
                                        })
                                        .last()
                                        .unwrap_or_default();
...
fn descriptor(
    ...
    ComponentDescriptor {
        ...
        materializer: Arc::new(Materializer(kind)),
    }
}
            ...
            MethodDescriptor::new(
                ...
                |args| match args {
                    [ComponentArgument::Boolean(v)] => {
                        Ok(ComponentPayload::new(Op::ShowCancel(*v)))
                    }
    ...
    registry.register(descriptor(
        ...
        vec![
            ...
            MethodDescriptor::new(
                ...
                vec![ArgumentDescriptor::new("value", ArgumentSchema::Boolean)],
                |args| match args {
                    [ComponentArgument::Boolean(v)] => Ok(ComponentPayload::new(Op::Autohide(*v))),
