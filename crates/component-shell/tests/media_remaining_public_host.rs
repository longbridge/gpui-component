#[path = "../src/shell/media_remaining/mod.rs"]
mod media_remaining;
...
impl gpui_shell::ComponentMaterializer for EditorProbe {
    fn materialize(
        ...
        let argument = &request
            ...
            .0;
        let state = request
            .with_state::<Entity<gpui_shell::gpui_component::input::EditorState>, _>(
                argument,
                Clone::clone,
            )?;
...
fn register_probe(registry: &mut gpui_shell::ComponentRegistry) {
    registry
        .register(gpui_shell::ComponentDescriptor {
            ...
            methods: vec![],
            typescript: gpui_shell::TypeScriptDescriptor::new("Test-only retained state probe."),
            materializer: Arc::new(EditorProbe),
        })
        .unwrap();
}
...
fn mount(
    ...
    media_remaining::register(&mut registry).unwrap();
