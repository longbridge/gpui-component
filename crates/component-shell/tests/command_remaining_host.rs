#[path = "../src/shell/command_remaining/mod.rs"]
...
fn mount(
    ...
    command_remaining::register(&mut registry).unwrap();
    let runtime =
    ...
    let window = cx.add_window(move |window, cx| {
        ...
        *slot.borrow_mut() = Some(view.clone());
        gpui_shell::gpui_component::Root::new(view, window, cx)
    });
...
"#;
    ...
    let mount_builds = command_remaining::command_probe::take();
    assert_builds("mount", &mount_builds, 1, 1, 5);
    draw(&mut context);
    let first_draw_builds = command_remaining::command_probe::take();
    assert_builds("first draw", &first_draw_builds, 1, 1, 5);
    draw(&mut context);
    let second_draw_builds = command_remaining::command_probe::take();
    ...
    command_remaining::command_probe::take();
    context.update(|_, cx| view.update(cx, |view, cx| view.refresh(cx)));
    draw(&mut context);
    let refresh_builds = command_remaining::command_probe::take();
...
"#;
    let (mut context, view, _app) = mount(cx, source);
    draw(&mut context);
    command_remaining::test_probe::take_shown();
