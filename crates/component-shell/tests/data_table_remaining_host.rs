#[path = "../src/shell/data_table_remaining/mod.rs"]
...
fn mount(
    ...
    data_table_remaining::register(&mut registry).unwrap();
    let runtime =
...
fn retained_data_table_renders_lazy_cells_from_plain_rows(cx: &mut TestAppContext) {
    data_table_remaining::test_probe::reset();
    let source = r#"
...
"#;
    ...
    assert!(data_table_remaining::test_probe::cell_builds() >= 4);
}
...
"#,
    ...
    context.update(|_, cx| assert_eq!(view.read(cx).build_error(), None));
    assert_eq!(data_table_remaining::test_probe::cell_builds(), 0);
    assert_eq!(data_table_remaining::test_probe::errors(), 1);
}
