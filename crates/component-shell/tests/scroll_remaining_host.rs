#[path = "../src/shell/scroll_remaining/mod.rs"]
...
fn mount(
    ...
    scroll_remaining::register(&mut registry).unwrap();
    let runtime =
...
"#;
    ...
    let handle = scroll_remaining::test_probe::latest_handle();
    ...
    draw(&mut context);
    let after_refresh = scroll_remaining::test_probe::latest_handle();
    assert_eq!(
...
export default class App extends View {{ init() {{ this.h = ScrollbarHandle(); }} render() {{ return {expression}; }} }}"#
    ...
    scroll_remaining::test_probe::take_errors();
    draw(&mut context);
    scroll_remaining::test_probe::take_errors()
}
