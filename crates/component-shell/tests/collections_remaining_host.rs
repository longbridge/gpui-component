#[path = "../src/shell/collections_remaining/mod.rs"]
...
fn mount(
    ...
    collections_remaining::register(&mut registry).unwrap();
    let runtime =
...
fn collection_catalog_stays_bounded() {
    let mut r =
        gpui_shell::ComponentRegistry::new(gpui_shell::COMPONENT_REGISTRY_API_VERSION).unwrap();
    collections_remaining::register(&mut r).unwrap();
...
"#;
    ...
    collections_remaining::test_probe::take_rows();
    draw(&mut context, view.clone());
    context.update(|_, cx| {
        assert_eq!(view.read(cx).build_error(), None);
    });
    let initial = distinct_rows(collections_remaining::test_probe::take_rows());
    assert_eq!(
        initial,
        [
    ...
    collections_remaining::test_probe::take_rows();
    draw(&mut context, view.clone());
    let collapsed = distinct_rows(collections_remaining::test_probe::take_rows());
    assert_eq!(
        collapsed,
        [
    ...
    let synced = distinct_rows(collections_remaining::test_probe::take_rows());
    assert_eq!(
        synced,
        [
    ...
    let expanded = distinct_rows(collections_remaining::test_probe::take_rows());
    assert_eq!(
        expanded,
        [
            ("src".into(), "Sources renamed".into(), true),
...
export default class App extends View {{ render() {{ return {expression}; }} }}"#
    ...
    let (mut context, view, _app) = mount(cx, &source);
    collections_remaining::test_probe::take_errors();
    draw(&mut context, view);
    collections_remaining::test_probe::take_errors()
}
