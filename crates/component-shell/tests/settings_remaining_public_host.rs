#[path = "../src/shell/settings_remaining/mod.rs"]
...
fn register_marker(registry: &mut gpui_shell::ComponentRegistry) {
    registry
        .register(gpui_shell::ComponentDescriptor {
            ...
            methods: vec![],
            typescript: gpui_shell::TypeScriptDescriptor::new(
                "Test-only lazy materialization marker.",
            ),
            materializer: Arc::new(LazyMarker),
        })
        .unwrap();
}
...
fn mount(
    ...
    settings_remaining::register(&mut registry).unwrap();
