#[path = "../src/shell/window_effects_remaining/mod.rs"]
...
use gpui_shell::gpui_component::{Root, WindowExt as _};
use std::{
    cell::RefCell,
    fs,
    ops::Deref as _,
...
fn register_marker(registry: &mut gpui_shell::ComponentRegistry) {
    registry
        .register(gpui_shell::ComponentDescriptor {
            name: "EffectMarker",
            constructors: vec![gpui_shell::ConstructorDescriptor::new(
                "EffectMarker",
            ...
            methods: vec![],
            typescript: gpui_shell::TypeScriptDescriptor::new("Test-only lazy marker."),
            materializer: Arc::new(Marker),
        })
        .unwrap();
}
...
fn mount_isolated(
    ...
    window_effects_remaining::register(&mut registry).unwrap();
    let runtime =
...
"#,
    ...
    context.update(|window, cx| {
        window.dispatch_action(Box::new(gpui_shell::gpui_component::dialog::Cancel), cx)
    });
...
export default class App extends View {{ render() {{ return {}; }} }}"#,
        ...
        window_effects_remaining::test_probe::take_slot_rejections();
        context.update(|window, cx| window.draw(cx).clear(cx));
        context.update(|window, cx| window.draw(cx).clear(cx));
        context.update(|_, cx| assert_eq!(view.read(cx).build_error(), None));
        let errors = window_effects_remaining::test_probe::take_slot_rejections();
        assert!(
            errors
                .iter()
...
export default class App extends View { render() { return new Dialog("fail", "Fail", () => { throw new Error("reporter exploded"); }).content(new EffectMarker("fail")); } }"#;
    ...
    let mut context = VisualTestContext::from_window(*window.deref(), cx);
    context.update(|window, cx| window.draw(cx).clear(cx));
    window_effects_remaining::test_probe::take_reporter_failures();
    context.simulate_click(point(px(40.), px(16.)), Modifiers::default());
    context.update(|window, cx| window.draw(cx).clear(cx));
    let errors = window_effects_remaining::test_probe::take_reporter_failures();
