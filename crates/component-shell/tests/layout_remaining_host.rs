#[path = "../src/shell/layout_remaining/mod.rs"]
...
fn mount(
    ...
    layout_remaining::register(&mut registry).unwrap();
    let runtime =
...
#[test]
fn layout_remaining_catalog_has_closed_real_state_and_typed_layout_contracts() {
    let mut registry =
        gpui_shell::ComponentRegistry::new(gpui_shell::COMPONENT_REGISTRY_API_VERSION).unwrap();
    layout_remaining::register(&mut registry).unwrap();
