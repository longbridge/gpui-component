...
pub enum DockEvent {
    ...
    /// including each step of a split or dock resize, so a subscriber that
    /// writes to disk should debounce.
    LayoutChanged,
...
pub struct DockArea {
    ...
    locked: bool,
    /// The tab group filling the whole area, if one is.
    ///
    /// A zoom names a *container*, never the panel inside it. The old dock
    /// zoomed the `TabPanel`: `TabPanel` is the only thing that ever emitted
    /// `PanelEvent::ZoomIn` — `subscribe_panel` was handed `StackPanel`s too,
    /// but those never zoom — so `set_zoomed_in` only ever received a whole
    /// tab panel, and a zoomed panel kept its tab bar, its toolbar and its
    /// menu. That is where the control that zooms back out lives. Naming the
    /// panel instead would strip all of it and leave the user with no way
    /// back. The group is rendered whole through its own
    /// [`TabGroupRenderer`].
...
impl DockArea {
    ...
    /// renderer also supplies the [`TabGroupRenderer`] every tab group it
    /// builds will use.
...
impl DockArea {
    ...
    /// starting a group when the region is empty.
    ...
    fn add_panel_inner(
        ...
        // handle rather than keeping this one matters too: a panel registered
        // through `add_panel_view` and then named by `add_panel` would
        // otherwise be left behind a bare entity, costing it its title for a
        // call that otherwise did nothing.
        let previous = self.panels.insert(id, panel);

        // A dock is created to hold the panel: the size seeds a dock that does
        // not exist yet, and the slot of a group that has to be made.
...
impl DockArea {
    ...
    pub fn set_zoomed_in(&mut self, node: NodeId, window: &mut Window, cx: &mut Context<Self>) {
        self.set_zoom(Some(node), window, cx);
    }
    ...
    /// The tab group filling the area, if one is.
    ...
    fn set_zoom(&mut self, zoomed: Option<NodeId>, window: &mut Window, cx: &mut Context<Self>) {
    ...
    fn zoomed_view(&self) -> Option<AnyView> {
        Some(self.groups.get(&self.zoomed?)?.entity.clone().into())
    }
}
...
impl DockArea {
    ...
    fn reconcile(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let zoom_survives = self
            .zoomed
            .is_none_or(|node| self.groups.contains_key(&node));
        if !zoom_survives {
            self.set_zoom(None, window, cx);
        }
...
mod tests {
    ...
    ///
    /// The chrome is the point: a tab bar is drawn by the *group*, and it
    /// does not run if the area renders the bare panel instead, so what lands
    /// in this log says which of the two is on screen — a question no reading
    /// of `is_zoomed()` can answer.
