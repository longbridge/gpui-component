...
const TOOLTIP_TRIGGER_GAP: Pixels = px(6.);
...
fn tooltip_overlay_position(
    ...
    let above_bounds = Bounds::new(
        point(centered_x, trigger_bounds.top() - gap - tooltip_size.height),
        tooltip_size,
    );
    let below_bounds = Bounds::new(
        point(centered_x, trigger_bounds.bottom() + gap),
        tooltip_size,
    );

    let bottom_limit = (viewport_size.height - margin).max(margin);
    let available_above = (trigger_bounds.top() - margin - gap).max(px(0.));
    let available_below = (bottom_limit - trigger_bounds.bottom() - gap).max(px(0.));
...
mod tests {
    ...
    fn tooltip_overlay_position_prefers_above_when_space_allows() {
        ...
        assert_eq!(position.bounds.origin.y, px(44.));
        assert_eq!(
            position.bounds.bottom() + TOOLTIP_TRIGGER_GAP,
            trigger_bounds.top()
        );
    }
    ...
    fn tooltip_overlay_position_flips_below_near_top_edge() {
        ...
        assert_eq!(
            position.bounds.top(),
            trigger_bounds.bottom() + TOOLTIP_TRIGGER_GAP
        );
