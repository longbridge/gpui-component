//! The panel presentation traits, and the concrete handle that carries them
//! back across the renderer seam.
//!
//! `gpui-base` owns panel *behavior*: [`gpui_base::dock::PanelView`] answers
//! for the name, the id, closability, visibility, and the activation
//! callbacks. Everything a tab bar actually draws — the title, the tab name,
//! the toolbar, the ellipsis menu — is presentation and lives here.
//!
...
//! type.
//!
//! This lives in its own module rather than in `panel.rs` because the old dock
//! still owns the flat `dock::Panel` and `dock::PanelView` names. The cutover
//! collapses the two modules.

use std::{any::Any, sync::Arc};

use gpui::{
    AnyElement, AnyView, App, Context, Entity, FocusHandle, IntoElement, SharedString, WeakEntity,
    Window,
};
...
pub trait Panel: gpui_base::dock::Panel {
    ...
    /// still be zoomed by the keyboard action. A panel that means "never
    /// zoom" has to say so in both places.
...
///
/// Concrete on purpose. Base stores it as `Arc<dyn
/// gpui_base::dock::PanelView>`, and every renderer hook — the tab bar, the
/// tile drag bar — gets it back with [`Self::of`], which is an `Any` downcast
/// to this one type. That is the only recovery Rust offers: the sub-trait
/// object `Arc<dyn PanelView>` cannot be reconstructed from base's handle, but
/// a concrete wrapper around it can be.
#[derive(Clone)]
...
impl PanelHandle {
    ...
    /// straight to `DockLayout::panel`, say. A skin must draw something for
    /// that case; base's own [`gpui_base::dock::PanelView::panel_name`] is the
    /// fallback with no obligations on the panel author.
