...
use crate::backoffice::mcp::server::{McpServer, McpServerSnapshot, McpCapability, ResourceDefinition, ResourceTemplateDefinition};
...
impl McpServerLoader {
    ...
    pub async fn load_server(
        ...
        log::info!("Loading MCP server: {} ({:?})", config.name, config.transport);
    ...
    async fn connect_stdio(
        ...
        let program = command_parts.next().ok_or_else(|| {
            anyhow::anyhow!("Empty command string")
        })?;
    ...
    async fn initialize_capabilities(
        ...
        if let Some(resources_capability) = server_info.capabilities.resources {
            ...
            if subscribable {
                ...
                for resource_name in &config.subscribed_resources {
                    if let Some(resource) = resources.iter_mut()
                        .find(|r| r.resource.name == *resource_name) {
                        
                        log::debug!("Subscribing to resource: {} ({})", 
                        ...
                        match client.subscribe(SubscribeRequestParam {
                            ...
                            Ok(_) => {
                                ...
                                log::debug!("Successfully subscribed to resource: {}", resource.resource.name);
                            }
                            Err(e) => {
        ...
        log::debug!("Server capabilities initialized: {:?}", snapshot.capabilities);
        Ok((client_arc, snapshot))
    }

    /// 构建 HTTP 请求头
    fn build_headers(env_vars: &std::collections::HashMap<String, String>) -> anyhow::Result<HeaderMap<HeaderValue>> {
