...
impl HomePage {
    ...
    pub(crate) fn open_sftp_view(
        ...
        let subscription = cx.subscribe_in(&sftp_view, window, move |_this, _sftp, event: &SftpViewEvent, window, cx| {
            match event {
                SftpViewEvent::OpenLocalTerminal { working_dir } => {
                    ...
                    let existing = tab_container
                        ...
                        .tabs()
                        .iter()
                        .filter(|t| t.id().starts_with("local-terminal-") || t.id().starts_with("terminal-"))
                        .count();
                    let idx = if existing > 0 { Some(existing + 1) } else { None };
                ...
                SftpViewEvent::OpenSshTerminal { connection,working_dir } => {
                    ...
                    tab_container.update(cx, |tc, cx| {
                        let tab = TabItem::new(
                            tab_id,
                            "ssh",
                            cx.new(|cx| TerminalView::new_ssh_with_index(conn, idx, window, cx, Some(working_dir))),
                        );
                        tc.add_and_activate_tab_with_focus(tab, window, cx);
                    });
                }
    ...
    pub(crate) fn add_ai_chat_tab(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        window.defer(cx, move |window, cx| {
            tab_container.update(cx, |tc, cx| {
                tc.activate_or_add_tab_lazy("ai-chat", |win, cx| {
                    let ai_chat = cx.new(|x| {
                        ChatPanel::new(win, x)
                    });
