...
-- ============================================================================
-- 注意：如果已有 user_configs 表，用 ALTER TABLE 迁移
...
CREATE POLICY teams_select ON teams FOR SELECT USING (
    id IN (SELECT tm.team_id FROM team_members tm WHERE tm.user_id = auth.uid())
    );
...
CREATE POLICY team_members_insert ON team_members FOR INSERT WITH CHECK (
    team_id IN (SELECT t.id FROM teams t WHERE t.owner_id = auth.uid())
    );
...
-- DELETE: 团队 owner 可移除成员，成员可自己退出
CREATE POLICY team_members_delete ON team_members FOR DELETE USING (
    user_id = auth.uid()
        OR team_id IN (SELECT t.id FROM teams t WHERE t.owner_id = auth.uid())
    );
...
CREATE POLICY user_configs_select ON user_configs FOR SELECT USING (
    user_id = auth.uid()
    );
CREATE POLICY user_configs_insert ON user_configs FOR INSERT WITH CHECK (
    user_id = auth.uid()
    );
CREATE POLICY user_configs_update ON user_configs FOR UPDATE USING (
    user_id = auth.uid()
    );
CREATE POLICY user_configs_delete ON user_configs FOR DELETE USING (
    user_id = auth.uid()
    );
